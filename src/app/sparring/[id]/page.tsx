...
export default function SparringPage() {
  ...
  const scale = Math.max(0.7, 1 - (scrollY - 24) / 300);
  const opacity = Math.max(0, 1 - (scrollY - 24) / 100);
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center gap-2 justify-between">
        <div className="w-2/3 h-full p-2">
          {/* 헤더 */}
          <div className="flex items-center justify-between sticky top-24 z-10 py-2">
            <div className={`flex items-center gap-4 origin-center origin-left`} style={{ transform: `scale(${scale})` }}>
              {debate ? (
                <>
                  <div className="bg-muted aspect-square rounded-md w-20 h-20 relative overflow-hidden">
                    <Image
                      src={debate.img}
                      alt={debate.title}
                      fill
                ...
                <>
                  <div className="bg-muted aspect-square rounded-md w-20 h-20">
                    {/* TODO: 이미지 */}
                  </div>
